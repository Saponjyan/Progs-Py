...
'3629','3631','3632','3634','3635','3636','3637','3640','3648','3654',]
    ...
    stranica1=["","&page=3","&page=4","&page=5","&page=6","&page=7","&page=8","&page=9"]
    async def lentos(idd,le):
        for l in le:
            await bot.send_message(idd,l)
            sleep(0.2)
    ...
    def start1():
        ...
        for i in pars.mag1:
            ...
            mag={'Store':f'{i}'}
            for z in pars.stranica1:

                response = requests.post(f"https://lenta.com/promo/?sorting=ByPriority{z}",headers=pars.headers,cookies=mag)
                sleep(2)
                ...
                try:
                    ...
                    for card in cards:
                        ...
                        card=" ".join(card)
                        if int(procent)>pars.proc:
                            sms=zz+"\n"+card+" \n "+da
                            if sms not in pars.lenta2:
                                pars.lenta2.append(sms)
                            #print(sms)
                except:
                    sleep(1)
########################################################################################
    #def start2():
        for i in pars.mag1:
            mag={'Store':f'{i}'}
            for z in pars.stranica1:
                response = requests.post(f"https://lenta.com/promo/?sorting=ByPriority&nodeCode=ga4638d8e16b266a51b9906c290531afb{z}",headers=pars.headers,cookies=mag)
                sleep(2)
                soup = BeautifulSoup(response.text,"lxml")
                cards=soup.find_all("div",class_="sku-card-small-container")
                pages=soup.find_all("a",role="button") 
                ...
                try:
                    ...
                    for card in cards:
                        ...
                        if int(procent)>pars.proc:
                            sms=zz+"\n"+card+" \n "+da
                            if sms not in pars.lenta3:
                                pars.lenta3.append(sms)
                            #print(sms)
                except:
                    sleep(1)
#########################################################################################
    #def start3():
        for i in pars.mag1:
            mag={'Store':f'{i}'}
            for z in pars.stranica1:
                response = requests.post(f" https://lenta.com/promo/?sorting=ByPriority&nodeCode=c565ed80964136a0374429fdc4d59ff8b{z}",headers=pars.headers,cookies=mag)
                sleep(2)
                soup = BeautifulSoup(response.text,"lxml")
                cards=soup.find_all("div",class_="sku-card-small-container")
                pages=soup.find_all("a",role="button") 
                ...
                try:
                    ...
                    for card in cards:
                        ...
                        procent=int(card[0][:-1])
                        card=" ".join(card)
                        if procent>pars.proc:
                            sms=zz+"\n"+card+" \n "+da
                            if sms not in pars.lenta4:
                                pars.lenta4.append(sms)
                            #print(sms)
